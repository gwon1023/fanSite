            ...
            <div className="relative grid gap-8">
              <div className="grid gap-3 md:grid-cols-2">
                {siteContent.identity.keywords.map((keyword, index) => (
                  <motion.span
                    key={keyword}
                    className="flex min-h-[4rem] items-center rounded-[1.25rem] border px-4 py-3 text-[0.72rem] uppercase tracking-[0.24em]"
                    style={{
              ...
              <div className="grid gap-6 md:grid-cols-[1.05fr_0.95fr]">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {siteContent.identity.labels.map((label) => (
                    <span
                      key={label}
                      className="flex min-h-[3.6rem] items-center rounded-[1.1rem] border border-white/10 bg-white/[0.03] px-3 py-2 text-[0.68rem] uppercase tracking-[0.28em] text-[var(--color-text-secondary)]"
                    >
                      {label}
                    </span>
                  ))}
